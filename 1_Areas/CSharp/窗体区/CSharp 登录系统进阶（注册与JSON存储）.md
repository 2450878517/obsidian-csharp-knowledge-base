---
title: 'C# 登录系统进阶（注册与 JSON 存储）'
tags: [CSharp, WinForms, 登录系统, JSON, 序列化]
status: budding
area: CSharp
aliases: [注册登录, JSON多对象, 多对象序列化, AppendText覆盖, 账号重复校验]
related: [CSharp JSON 序列化（Newtonsoft.Json）, CSharp 登录系统（窗体跳转与密码隐藏）, CSharp 列表 List, CSharp编程入门_视频笔记, CSharp 选择控件（RadioButton 与 ListBox）]
---
# C# 登录系统进阶（注册与 JSON 存储）

[[CSharp JSON 序列化（Newtonsoft.Json）|C# JSON 序列化（Newtonsoft.Json）]] 讲了**单个对象**怎么序列化保存；[[CSharp 登录系统（窗体跳转与密码隐藏）|C# 登录系统（窗体跳转与密码隐藏）]] 讲了账号密码**写死在代码里**的登录。这课把两件事合起来：**一次存多个对象（注册的账号一堆人）**，再把登录系统改成**注册的信息存本地 JSON 文件**，真的能加人、能登录。
...
- 「登录」按钮 + 「注册」按钮
- 一个 CheckBox「显示密码」，勾上显明文、取消又藏起来（和 [[CSharp 登录系统（窗体跳转与密码隐藏）|C# 登录系统（窗体跳转与密码隐藏）]] 一样）
...
**相关笔记：** [[CSharp JSON 序列化（Newtonsoft.Json）|C# JSON 序列化（Newtonsoft.Json）]] | [[CSharp 登录系统（窗体跳转与密码隐藏）|C# 登录系统（窗体跳转与密码隐藏）]] | [[CSharp 列表 List|C# 列表 List]] | [[CSharp编程入门_视频笔记#🎬 8月11日下午 — 文件夹与文件操作（增删改查 & TXT读写 & 打包）|C# 文件与文件夹操作]] | [[CSharp 窗体控件基础|C# 窗体控件基础]] | [[CSharp编程入门_视频笔记|C#编程入门_视频笔记]]
